...
export const VolunteerApplicationForm: React.FC<VolunteerApplicationFormProps> = ({ onClose }) => {
  ...
  const validate = () => {
    const newErrors = { fullName: '', email: '', interest: '' };
    let isValid = true;
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (validate()) {
      ...
      fetch("/", {
          ...
          body: encode({ "form-name": "volunteer-application", ...formData })
      })
      .then(() => {
          setIsSubmitted(true);
  ...
  const isFormInvalid = !formData.fullName || !formData.email || wordCount < MIN_WORDS || wordCount > MAX_WORDS;

  return (
    <form name="volunteer-application" data-netlify="true" onSubmit={handleSubmit} noValidate>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="flex justify-between mt-1 text-xs">
            {errors.interest && <p className="text-red-600">{errors.interest}</p>}
